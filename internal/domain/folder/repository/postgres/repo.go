...
func (r repo) GetByID(ctx context.Context, id ulid.ULID) (*folder.Folder, error) {
	...
	var f sqlcommon.FolderDTO

	err := r.db.Gorm(ctx).
		Model(&sqlcommon.FolderDTO{}).
		Where("folders.id = ?", id).
		First(&f).Error
	if err != nil {
	...
	return sqlcommon.AsModel(&f), nil
}
...
func (r repo) getByRegexp(tx *gorm.DB, accountID ulid.ULID, f folder.Filter, order folder.Order, regex *regexp.Regexp) ([]sqlcommon.FolderDTO, error) {
	prefix, complete := regex.LiteralPrefix()

	var reDTO []sqlcommon.FolderDTO
...
func (r repo) GetByPrefix(ctx context.Context, accountID ulid.ULID, f folder.Filter, prefixes ...string) ([]folder.Folder, error) {
	...
	var dtoMap map[ulid.ULID]sqlcommon.FolderDTO

	err := r.db.Gorm(ctx).Transaction(func(tx *gorm.DB) error {
		for _, p := range prefixes {
			var dtos []sqlcommon.FolderDTO
	...
	for _, dto := range dtoMap {
		models = append(models, *sqlcommon.AsModel(&dto))
	}
...
func (r repo) CreateEntry(ctx context.Context, entry ...folder.Entry) error {
	...
	dtos := make([]sqlcommon.EntryDTO, len(entry))
	for i, ent := range entry {
		dtos[i] = *sqlcommon.EntryAsDTO(&ent)
	}
	...
	err := r.db.Gorm(ctx).Transaction(func(tx *gorm.DB) error {
		for _, ent := range old {
			err := r.db.Gorm(ctx).
				...
				Delete(&sqlcommon.EntryDTO{}).Error
			if err != nil {
				return err
			}
		}
		for _, ent := range new {
			err := r.db.Gorm(ctx).Create(sqlcommon.EntryAsDTO(&ent)).Error
			if err != nil {
				return err
			}
		}
...
func (r repo) GetEntryByUIDRange(ctx context.Context, folderID ulid.ULID, ranges ...folder.UIDRange) ([]folder.Entry, error) {
	...
	if len(ranges) == 1 {
		var entries []sqlcommon.EntryDTO
		...
		for _, ent := range entries {
			models = append(models, *sqlcommon.EntryAsModel(&ent))
		}
		return models, nil
	} else {
		entryMap := make(map[uint32]sqlcommon.EntryDTO)
